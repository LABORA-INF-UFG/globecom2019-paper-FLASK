...
class ExtensionFunction {
 public:
  inline bool operator()(const Graph& g, ResourceContainer& new_cont,
    ...
    new_cont.profit =
        old_cont.profit +
        ((eta_ * global_interest[(poic_global->GetPOIById(vert_prop_t.poi_id))
                                     ->GetCategory()]) +
         ((1 - eta_) *
...
TourContainer* SPPRCSolver::Solve(POIContainer* poic, User* u, Tour* s,
  ...
  add_edge(0, firstPOIId, EdgeProperty(1, 0), g);
  ...
  for (int i = 1, edge_num = 1; i <= poic->GetNumOfPOIs(); i++) {
    ...
    for (int j = 1; j <= poic->GetNumOfPOIs(); j++) {
      ...
      add_edge(
          i, j,
          EdgeProperty(edge_num++, poic->GetTimeBetweenPOIs(i, j, "walking")),
          g);
    }
  }
  ...
  for (int i = 0; i < static_cast<int>(opt_solutions.size()); i++) {
    if (pareto_opt_rcs[i].path.size() < 2) continue;
    // cout << pareto_opt_rcs[i].path.size() << " " <<
    // pareto_opt_rcs[i].travel_spent.size() << endl;
    Tour* new_s = new Tour(s->GetSeqId());
    vector<int> allocation_in_time;
    vector<vector<int> > allocation_in_time_at_poi;

    allocation_in_time_at_poi.assign(poic->GetNumOfPOIs() + 1, vector<int>());
    for (int j = 0; j <= poic->GetNumOfPOIs(); j++)
      allocation_in_time_at_poi[j].assign(481, 0);

    allocation_in_time.assign(481, 0);
    new_s->SetProfit(pareto_opt_rcs[i].profit);
    double aux = 0;

    for (int j = 0; j < pareto_opt_rcs[i].path.size(); j++) {
      POI* p = poic->GetPOIById(pareto_opt_rcs[i].path[j]);
      ...
      if (j + 1 < pareto_opt_rcs[i].travel_spent.size()) {
        // cout << pareto_opt_rcs[i].travel_spent[j];
