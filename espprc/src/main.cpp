...
void process(string filename) {
  ...
  i_file.open(
      ("../../data/" + filename_lower + "/userVisits-" + filename + ".csv")
          .c_str());
  o_file.open(
      ("../../data/" + filename_lower + "/UserVisits" + filename + ".in")
          .c_str());
  ...
  o_file.close();
  i_file.open(
      ("../../data/" + filename_lower + "/POI-" + filename + ".csv").c_str());
  o_file.open(
      ("../../data/" + filename_lower + "/POI" + filename + ".in").c_str());
  ...
  i_file.close();
  o_file.close();
  i_file.open(("../../data/" + filename_lower + "/costProfCat-" + filename +
               "POI-all.csv")
                  .c_str());
  o_file.open(("../../data/" + filename_lower + "/CostProft" + filename + ".in")
                  .c_str());
...
void compare(string city_name, Tour* s1, Tour* s2, POIContainer* poic,
  ...
  if (s2->GetSeqLength() == 0) {
    ...
    for (int i = 1; i <= poic->GetNumOfPOIs(); i++) {
      if (visited_s1[i] == visited_s2[i] && visited_s1[i] == 1) intersec_size++;
    }
...
void saveTour(POIContainer* poic, User* u, Tour* s) {
  ...
  for (int i = 1; i <= poic->GetNumOfPOIs(); i++) {
    for (int j = i + 1; j <= poic->GetNumOfPOIs(); j++) {
      if (i != j) {
        if (i == poic->GetNumOfPOIs() - 1 && j == poic->GetNumOfPOIs()) {
          o_file << poic->GetTimeBetweenPOIs(i, j, "walking") << endl;
        } else {
          o_file << poic->GetTimeBetweenPOIs(i, j, "walking") << "," << endl;
        }
      }
  ...
  for (int i = 1; i <= poic->GetNumOfPOIs(); i++) {
    ...
    if (i == poic->GetNumOfPOIs()) {
      o_file << i << ": " << tvisit << endl;
    } else {
      o_file << i << ": " << tvisit << "," << endl;
    }
  }
...
void load(CityContainer* c_container, string city) {
  ...
  file.open(("../../data/" + city_lower + "/pois.in").c_str());
  ...
  while (file >> id) {
    ...
    file >> popularity;
    file >> category;
    p_container->AddPoi(new POI(id, name, lat, lng, category, popularity));
  }
  ...
  file.open(("../../data/" + city_lower + "/pois_travel_time.in").c_str());

  while (file >> p1_id) {
    file >> p2_id;
    file >> value;
    // cout << p1_id << " " << p2_id << " " << value << endl;
  ...
  file.open(("../../data/" + city_lower + "/user_visits.in").c_str());
...
int main(int argc, char* argv[]) {
  int num_of_cities = 4;
  string cities[] = {"london", "madrid", "barcelona", "athens"};
  int total = 0;
  double eta;
  int interest_type;
  ...
  if (argc != 4) {
    cout
        << "Uso: tourist -rotas eta_value interest_type, para gerar as rotas, "
           "ou tourist -solve eta_value interest_type, para resolver o problema"
        << endl;
    return -1;
  }
  ...
  for (int k = 0; k < c_container->GetNumOfCities(); k++) {
    ...
    processados = 0;
    if (eta == 0) {
      result_file.open(("../../results/espprc/" + to_lower(c->GetName()) +
    ...
    while (processed_itineraries < 500) {
      ...
      // cout << user_index << " " << tour_index << endl;
    ...
    result_file.close();
    ofstream tour_file;
    tour_file.open("../../data/" + to_lower(c->GetName()) + "/tour_file.in",
                   fstream::out);
    ...
    for (int i = 0; i < generated_solutions.size(); i++) {
      ...
      for (int j = 0; j < generated_solutions[i]->GetNumOfTours(); j++) {
        tour_file << generated_solutions[i]->GetTour(j)->GetSeqId() << endl;
        ...
        tour_file << allocation_history[480] << endl;
        tour_file << p_container->GetNumOfPOIs() << endl;
        for (int l = 1; l <= p_container->GetNumOfPOIs(); l++) {
          for (int k = 0; k < 480; k++) {
            if (allocation_history_at_poi[l][k] == 340)
              tour_file << 1 << " ";
            else
              tour_file << allocation_history_at_poi[l][k] << " ";
          }
          tour_file << allocation_history_at_poi[l][480] << endl;
        }
      }
    }
    tour_file.close();
